...
bool find_motion(int j, int k, int id, float *** dist_1){
    if(k>10){
        if((abs(dist_1[j][id][k-10]-dist_1[j][id][k])>0.05)&&(dist_1[j][id][k-10]!=0)){//uslovia rassmartivat po osyam a ne po dist
//            cout<<"id: "<<id<<" k: "<< k <<endl <<"dist bilo:"<<dist_1[j][id][k-5]<<endl<<"dist stalo: "<< dist_1[j][id][k]<<endl;;
            return true;
        }
    }
    return false;
}
//((tvec_1[0]-tvec_2[0])^2+(tvec_1[1]-tvec_2[1])^2+(tvec_1[2]-tvec_2[2])^2)>0.05)
...
//                cout<<ids_to_print_3.str()<<endl<<ids_to_print_3.str();
            }
            for (int i = 0; i<ids.size(); i++){
        ...
        case 3:{
            if (ids.size()>0){
                ...
                ids_to_print_4 << all_ids_4.back();
//                cout<<ids_to_print_4.str()<<endl<<ids_to_print_4.str();
            }
            for (int i = 0; i<ids.size(); i++){
...
void draw_cylinder(vector<int> ids_cylinder, int j, vector<Vec3d> tvecs, Affine3d pose,Viz3d myWindow, Viz3d secondWindow){
    switch (j){
        case 0:{
            ...
            myWindow.showWidget("cylinder_"+to_string(ids_cylinder[i])+"from_dict"+to_string(j), WCylinder(Point3d(x,y,z),Point3d(x-0.1,y-0.2,z-length_cylinder), radius, 20, viz::Color::white()));
            }
            break;
        }
        case 1:{
            ...
            for (int i =0; i<ids_cylinder.size(); i++){
                Vec3d tvec = tvecs[i];
                float x = tvec[0], y = tvec[1], z = tvec[2];
            myWindow.showWidget("cylinder_"+std::to_string(ids_cylinder[i])+"from_dict"+to_string(j), WCylinder(Point3d(x,y,z),Point3d(x-0.1,y-0.2,z-length_cylinder), radius, 20, viz::Color::white()));
            }
            break;
        }
        case 2:{
...
//    Affine3f camerapose = viz::WCameraPosition(cameraMatrix);
    ...
    Mat image;
    VideoCapture cap;
    const char* video = "/Users/grashin/video_detection/samsung_videos/samsung_23.mp4";
    cap.open(video);
...
//                Pt3D_Origin(0.0,0.0,0.0);
    ...
    vector<Vec3d> tvec_for_detect[4];
    vector<vector<Vec3d>> vector_of_tvec_5(4, vector<Vec3d>(5));
//    int c = 0;
    while (cap.grab()) {
        Mat image, imageCopy;
        cap.retrieve(image);
...
//                vector_of_tvec_5[j][c%5] = tvecs;
//                tvec_for_detect[j] = vector_of_tvec_5[j].begin()
//                c++;
//                vector_of_tvec_5[j].erase(vector_of_tvec_5[j].begin());
                for(int i = 0; i< 21; i++){
                    k_sum += k[j][i];
                }
                if(j == 2){
    ...
    //            myWindow.showWidget("trajectory", viz::WTrajectory(pose));
                ...
                Affine3d pose_10(-h, Vec3f(e,b,w+0.5));
...
//                Affine3d point_pose(Mat::eye(3, 3, CV_64F), Pt3d_Object);
    ...
    //                cout<<point_in_cw;
                ...
                ids_print(k_sum, j, ids, secondWindow);
    ...
    //                drawFrameAxes(imageCopy, K, distCoeffs, rvec, tvec, 0.3);
                    ...
                    y = tvec[1];
                    z = tvec[2];
                    dist_1[j][ids[i]][k[j][i]] = sqrt(x*x+y*y+z*z);
//                    dist.push_back(sqrt(x*x+y*y+z*z));
                    string str = to_string(floor(100*dist_1[j][ids[i]][k[j][i]])/100);
//                    print_vector(dist);
//                    bool fdf = find_motion(j, k[j], ids[i], tvec_for_detect[j][i], tvecs[i]);
                    bool is_move = find_motion(j, k[j][i], ids[i], dist_1);
                    if(is_move){
                        colvo[j][ids[i]]+=1;
                        if(colvo[j][ids[i]]  == 1){
                            string move = "move! id: " + to_string(ids[i]) + " from dict: " + to_string(j);
                            char const *pchar_10 = move.c_str();
                            secondWindow.showWidget("move"+to_string(ids[i])+"from_dist" + to_string(j), WText(pchar_10, Point2f(500.0,50.0+j*30), 30.0, viz::Color::white()));
                            cout<< "move!!!"<<endl<<"id: "<<ids[i]<<"  dictionary: "<<j<<endl;
                        }
                    }
                    string dist = "Dist" + to_string(ids[i]) +"_"+to_string(j)+ ": " + str.substr(0, str.find_last_not_of('0') + 1);
                    char const *pchar = dist.c_str();
                    secondWindow.showWidget("dist_"+to_string(ids[i])+"_from_dict_"+to_string(j), WText(pchar,Point2f(1100.0, ids[i]*20+40*j), 20.0, viz::Color::white() ));
...
//        myWindow.showWidget("cylinder_jhvjhvjh", WCylinder(Point3d(1,3,1),Point3d(1,2,2), 0.2, 20, viz::Color::white()));
